...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    root.title('Sweep and Analyze Box Plots')

    # This function runs faster if placed after initializing tk window root than before it
    def get_path():
        # global dataframe
        
        excel_folder_path = filedialog.askdirectory(title="Select a folder to save Excel workbook")  # Get the path from file dialog
        print(f'Folder where Excel workbook will be saved: {excel_folder_path}')

        if excel_folder_path == '':
            excel_folder_path == 'Please choose a file path for where to save Excel workbook'
            path_label.config(text=excel_folder_path)  # Update the path label text

            return
        else:
            path_label.config(text=excel_folder_path)
    ...
    # Device Batch entry
    ttk.Label(root, text="Device Architecture:").grid(row=0, column=0, padx=10, pady=5)
    device_batch_entry = ttk.Entry(root) # convert to float when .get()
    device_batch_entry.grid(row=0, column=1, padx=10, pady=5)

    # Device number entry
    ttk.Label(root, text="Device Number:").grid(row=1, column=0, padx=10, pady=5)
    device_number_entry = ttk.Entry(root) # convert to float when .get()
    device_number_entry.grid(row=1, column=1, padx=10, pady=5)

    # Device surface area entry
    ttk.Label(root, text="Device Surface Area (cm\u00b2):").grid(row=2, column=0, padx=10, pady=5)
    device_area_entry = ttk.Entry(root) # convert to float when .get()
    device_area_entry.grid(row=2, column=1, padx=10, pady=5)

    # Minimum voltage entry
    ttk.Label(root, text="Minimum Voltage (V):").grid(row=3, column=0, padx=10, pady=5)
    min_voltage_entry = ttk.Entry(root) # convert to float when .get()
    min_voltage_entry.grid(row=3, column=1, padx=10, pady=5)

    # Maximum voltage entry
    ttk.Label(root, text="Maximum Voltage (V):").grid(row=4, column=0, padx=10, pady=5)
    max_voltage_entry = ttk.Entry(root)
    max_voltage_entry.grid(row=4, column=1, padx=10, pady=5)

    # Choose file button
    choose_file_btn = ttk.Button(root, text="Choose Path", command=get_path)
    choose_file_btn.grid(row=5, column=2, padx=10, pady=5)
    ...
    2. Select minimum and maximum voltage for sweeps (e.g., )"""
